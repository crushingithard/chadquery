...
test( "queue() with other types", 14, function() {
	...
	$div
		.queue("foo",function(){
			...
			chadQuery.dequeue(this,"foo");
		})
		.queue("foo",function(){
			equal( ++counter, 2, "Dequeuing" );
			chadQuery(this).dequeue("foo");
		})
		.queue("foo",function(){
...
test("queue() passes in the next item in the queue as a parameter to fx queues", function() {
	...
	chadQuery.when( div.promise("fx"), div ).done(function() {
...
test("callbacks keep their place in the queue", function() {
	...
	div.queue(function( next ) {
		...
		chadQuery( this ).delay( 100 ).queue(function( next ) {
...
test( "chadQuery.queue should return array while manipulating the queue", 1, function() {
	var div = document.createElement("div");

	ok( chadQuery.isArray( chadQuery.queue( div, "fx", chadQuery.noop ) ), "chadQuery.queue should return an array while manipulating the queue" );
});
...
test("clearQueue() clears the fx queue", function() {
	...
	div.queue(function( next ) {
		counter++;
		var self = this;
		setTimeout(function() { chadQuery(self).clearQueue(); next(); }, 50);
	}).queue(function() {
		counter++;
	});
...
test( ".promise(obj)", function() {
	...
	var obj = {},
		promise = chadQuery( "#foo" ).promise( "promise", obj );

	ok( chadQuery.isFunction( promise.promise ), ".promise(type, obj) returns a promise" );
	strictEqual( promise, obj, ".promise(type, obj) returns obj" );
});


if ( chadQuery.fn.stop ) {
