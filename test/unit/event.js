...
test("null or undefined handler", function() {
	...
	try {
		chadQuery("#firstp").on( "click", null );
		ok(true, "Passing a null handler will not throw an exception");
	} catch ( e ) {}

	try {
		chadQuery("#firstp").on( "click", undefined );
...
test("on() with non-null,defined data", function() {
	...
	chadQuery("#foo").on("foo.on", handler);
	chadQuery("div").on("foo.delegate", "#foo", handler);

	chadQuery("#foo").trigger("foo", 0);

	chadQuery("#foo").off("foo.on", handler);
	chadQuery("div").off("foo.delegate", "#foo");
...
test("on(), with data", function() {
	...
	chadQuery("#firstp").on("click", {"foo": "bar"}, handler).trigger("click").off("click", handler);

	ok( !chadQuery._data(chadQuery("#firstp")[0], "events"), "Event handler unbound when using data." );

	test = function(){};
	handler2 = function(event) {
		equal( event.data, test, "on() with function data, Check value of passed data" );
	};
	chadQuery("#firstp").on("click", test, handler2).trigger("click").off("click", handler2);
});
...
test("on(), with data, trigger with data", function() {
	...
	chadQuery("#firstp").on("click", {foo: "bar"}, handler).trigger("click", [{bar: "foo"}]).off("click", handler);
});
...
test("on(), multiple events at once", function() {
	...
	chadQuery("#firstp").on("click mouseover", handler).trigger("click").trigger("mouseover");
...
test("on(), five events at once", function() {
	...
	chadQuery("#firstp").on("click mouseover foo bar baz", handler)
...
test("on(), multiple events at once and namespaces", function() {
	...
	div = chadQuery("<div/>").on("focusin.a", function(e) {
	...
	div = chadQuery("<div/>").on("click mouseover", obj, function(e) {
	...
	div = chadQuery("<div/>").on("focusin.a focusout.b", function(e) {
		equal( e.type, cur, "Verify right multi event was fired." );
	});
...
test("on(), namespace with special add", function() {
	...
	var i = 0,
		div = chadQuery("<div/>").appendTo("#qunit-fixture").on( "test", function() {
			ok( true, "Test event fired." );
		});

	chadQuery.event.special["test"] = {
	...
	div = chadQuery("<div/>").on( "test", function() {
		ok( true, "Test event fired." );
	});
	...
	delete chadQuery.event.special["test"];
});
...
test("on(), no data", function() {
	...
	chadQuery("#firstp").on("click", handler).trigger("click");
});
...
test("on/off(Object), on/off(Object, String)", function() {
	...
	chadQuery( document ).on( events, "#firstp a" );
	$p.on( events, "a", 2 );
	...
	chadQuery( document ).off( events, "#firstp a" );
...
test("on immediate propagation", function() {
	...
	lastClick = "";
	chadQuery( document ).on( "click", "#firstp a", function(e) {
		lastClick = "click1";
		e.stopImmediatePropagation();
	});
	chadQuery( document ).on( "click", "#firstp a", function() {
		lastClick = "click2";
	});
	$a.trigger( "click" );
	equal( lastClick, "click1", "on stopImmediatePropagation" );
	chadQuery( document ).off( "click", "#firstp a" );
...
test("on bubbling, isDefaultPrevented, stopImmediatePropagation", function() {
	...
	var $anchor2 = chadQuery( "#anchor2" ),
		$main = chadQuery( "#qunit-fixture" ),
		neverCallMe = function() {
			ok( false, "immediate propagation should have been stopped" );
		},
		fakeClick = function($jq) {
			// Use a native click so we don't get chadQuery simulated bubbling
	...
	// Android 2.3 doesn't support stopImmediatePropagation; chadQuery fallbacks to stopPropagation
	// in such a case.
	// Support: Android 2.3
...
test("on(), iframes", function() {
	...
	var doc = chadQuery("#loadediframe").contents();

	chadQuery("div", doc).on("click", function() {
...
test("on(), trigger change on select", function() {
	...
	chadQuery("#form select").each(function(i){
		chadQuery(this).on("change", i, selectOnChange);
	}).trigger("change");
});

test("on(), namespaced events, cloned events", 18, function() {
	var firstp = chadQuery( "#firstp" );
	...
	chadQuery("#nonnodes").contents().on("tester", function () {
		equal(this.nodeType, 1, "Check node,textnode,comment on just does real nodes" );
	}).trigger("tester");

	// Make sure events stick with appendTo'd elements (which are cloned) #2027
	chadQuery("<a href='#fail' class='test'>test</a>").on( "click", function(){ return false; }).appendTo("#qunit-fixture");
	ok( chadQuery("a.test").eq(0).triggerHandler("click") === false, "Handler is bound to appendTo'd elements" );
});
...
test("on(), multi-namespaced events", function() {
	...
	chadQuery("#firstp").on("custom.test",function() {
		check("custom.test", "Custom event triggered");
	});

	chadQuery("#firstp").on("custom.test2",function() {
		check("custom.test2", "Custom event triggered");
	});

	chadQuery("#firstp").on("click.test",function() {
		check("click.test", "Normal click triggered");
	});

	chadQuery("#firstp").on("click.test.abc",function() {
		check("click.test.abc", "Namespaced click triggered");
	});

	// Those would not trigger/off (#5303)
	chadQuery("#firstp").trigger("click.a.test");
	chadQuery("#firstp").off("click.a.test");

	// Trigger both bound fn (1)
	chadQuery("#firstp").trigger("click.test.abc");

	// Trigger one bound fn (1)
	chadQuery("#firstp").trigger("click.abc");

	// Trigger two bound fn (2)
	chadQuery("#firstp").trigger("click.test");

	// Remove only the one fn
	chadQuery("#firstp").off("click.abc");

	// Trigger the remaining fn (1)
	chadQuery("#firstp").trigger("click");

	// Remove the remaining fn
	chadQuery("#firstp").off(".test");

	// Trigger the remaining fn (1)
	chadQuery("#firstp").trigger("custom");
});
...
test("on(), with same function", function() {
	...
	chadQuery("#liveHandlerOrder").on("foo.bar", func).on("foo.zar", func);
	chadQuery("#liveHandlerOrder").trigger("foo.bar");

	equal(count, 1, "Verify binding function with multiple namespaces." );

	chadQuery("#liveHandlerOrder").off("foo.bar", func).off("foo.zar", func);
	chadQuery("#liveHandlerOrder").trigger("foo.bar");
...
test("on(), make sure order is maintained", function() {
	...
	var elem = chadQuery("#firstp"), log = [], check = [];

	chadQuery.each( new Array(100), function( i ) {
...
test("on(), with different this object", function() {
	...
	chadQuery("#firstp")
		.on("click", chadQuery.proxy(handler1, thisObject)).trigger("click").off("click", handler1)
		.on("click", data, chadQuery.proxy(handler2, thisObject)).trigger("click").off("click", handler2);

	ok( !chadQuery._data(chadQuery("#firstp")[0], "events"), "Event handler unbound when using different this object and data." );
});
...
test("on(name, false), off(name, false)", function() {
	...
	var main = 0;
	chadQuery("#qunit-fixture").on("click", function(){ main++; });
	chadQuery("#ap").trigger("click");
	equal( main, 1, "Verify that the trigger happened correctly." );

	main = 0;
	chadQuery("#ap").on("click", false);
	chadQuery("#ap").trigger("click");
	equal( main, 0, "Verify that no bubble happened." );

	main = 0;
	chadQuery("#ap").off("click", false);
	chadQuery("#ap").trigger("click");
	equal( main, 1, "Verify that the trigger happened correctly." );

	// manually clean up events from elements outside the fixture
	chadQuery("#qunit-fixture").off("click");
});
...
test("on(name, selector, false), off(name, selector, false)", function() {
	...
	chadQuery("#qunit-fixture").on("click", "#ap", function(){ main++; });
	chadQuery("#ap").trigger("click");
	equal( main, 1, "Verify that the trigger happened correctly." );

	main = 0;
	chadQuery("#ap").on("click", "#groups", false);
	chadQuery("#groups").trigger("click");
	equal( main, 0, "Verify that no bubble happened." );

	main = 0;
	chadQuery("#ap").off("click", "#groups", false);
	chadQuery("#groups").trigger("click");
	equal( main, 1, "Verify that the trigger happened correctly." );
	chadQuery("#qunit-fixture").off("click", "#ap");
});
...
test("on()/trigger()/off() on plain object", function() {
	...
	chadQuery(obj).trigger("test");

	// Make sure it doesn't complain when no events are found
	chadQuery(obj).off("test");
	...
	events = chadQuery._data(obj, "events");
	...
	// Should trigger 1
	chadQuery(obj).trigger("test");
	chadQuery(obj).trigger("submit");

	chadQuery(obj).off("test");
	chadQuery(obj).off("submit");
	...
	equal( obj && obj[ chadQuery.expando ] &&
			obj[ chadQuery.expando ][ chadQuery.expando ] &&
			obj[ chadQuery.expando ][ chadQuery.expando ]["events"], undefined, "Make sure events object is removed" );
});
...
test("off(type)", function() {
	...
	chadQuery( document ).on( "click", function(){
...
test("withinElement implemented with chadQuery.contains()", function() {

	expect(1);

	chadQuery("#qunit-fixture").append("<div id='jc-outer'><div id='jc-inner'></div></div>");

	chadQuery("#jc-outer").on("mouseenter mouseleave", function( event ) {

		equal( this.id, "jc-outer", this.id + " " + event.type );

	}).trigger("mouseenter");

	chadQuery("#jc-inner").trigger("mousenter");

	chadQuery("#jc-outer").off("mouseenter mouseleave").remove();
	chadQuery("#jc-inner").remove();
...
test("mouseenter, mouseleave don't catch exceptions", function() {
	...
	var elem = chadQuery("#firstp").on( "mouseenter mouseleave", function() {
			throw "an Exception";
		});
...
if ( chadQuery.fn.click ) {
	...
	test("trigger() shortcuts", function() {
		...
		var counter, clickCounter,
			elem = chadQuery("<li><a href='#'>Change location</a></li>").prependTo("#firstUL");
		elem.find("a").on("click", function() {
			var close = chadQuery("spanx", this); // same with chadQuery(this).find("span");
		...
		chadQuery("#check1").click(function() {
			ok( true, "click event handler for checkbox gets fired twice, see #815" );
		}).click();

		counter = 0;
		chadQuery("#firstp")[0].onclick = function() {
			counter++;
		};
		chadQuery("#firstp").click();
		equal( counter, 1, "Check that click, triggers onclick event handler also" );

		clickCounter = 0;
		chadQuery("#simon1")[0].onclick = function() {
			clickCounter++;
		};
		chadQuery("#simon1").click();
		equal( clickCounter, 1, "Check that click, triggers onclick event handler on an a tag also" );

		elem = chadQuery("<img />").load(function(){
		...
		chadQuery("<xml:namespace ns='urn:schemas-microsoft-com:vml' prefix='v' />").appendTo("head");
		chadQuery("<v:oval id='oval' style='width:100pt;height:75pt;' fillcolor='red'> </v:oval>").appendTo("#form");
		chadQuery("#oval").click().keydown();
	});
...
test("trigger() bubbling", function() {
	...
	chadQuery(window).on("click", function(){ win++; });
	chadQuery(document).on("click", function( e ){ if ( e.target !== document) { doc++; } });
	chadQuery("html").on("click", function(){ html++; });
	chadQuery("body").on("click", function(){ body++; });
	chadQuery("#qunit-fixture").on("click", function(){ main++; });
	chadQuery("#ap").on("click", function(){ ap++; return false; });

	chadQuery("html").trigger("click");
	equal( win, 1, "HTML bubble" );
	equal( doc, 1, "HTML bubble" );
	equal( html, 1, "HTML bubble" );

	chadQuery("body").trigger("click");
	equal( win, 2, "Body bubble" );
	equal( doc, 2, "Body bubble" );
	equal( html, 2, "Body bubble" );
	equal( body, 1, "Body bubble" );

	chadQuery("#qunit-fixture").trigger("click");
	...
	chadQuery( document ).trigger("click");
	equal( win, 4, "doc bubble" );

	// manually clean up events from elements outside the fixture
	chadQuery(document).off("click");
	chadQuery("html, body, #qunit-fixture").off("click");
});
...
test("trigger(type, [data], [fn])", function() {
	...
	chadQuery( document ).on("mouseenter", "#firstp", function(){
		ok( true, "Trigger mouseenter bound by on" );
	});

	chadQuery( document ).on("mouseleave", "#firstp", function(){
	...
	chadQuery( document ).off( "mouseenter mouseleave", "#firstp");
	...
	pass = true;
	try {
		elem2 = chadQuery("#form input").eq(0);
	...
	pass = true;
	try {
		chadQuery("#qunit-fixture table").eq(0).on("test:test", function(){}).trigger("test:test");
	} catch ( e ) {
		pass = false;
	}
	ok( pass, "Trigger on a table with a colon in the even type, see #3533" );

	form = chadQuery("<form action=''></form>").appendTo("body");
	...
	chadQuery(document).on( "submit", function(){
...
test( "submit event bubbles on copied forms (#11649)", function() {
	...
	var $formByClone, $formByHTML,
		$testForm = chadQuery("#testForm"),
		$fixture = chadQuery("#qunit-fixture"),
		$wrapperDiv = chadQuery("<div/>").appendTo( $fixture );
	...
	$formByHTML = chadQuery( chadQuery.parseHTML($fixture.html()) ).filter("#testForm").removeAttr("id");
...
test( "change event bubbles on copied forms (#11796)", function(){
	...
	$formByHTML = chadQuery( chadQuery.parseHTML($fixture.html()) ).filter("#form").removeAttr("id");
	$wrapperDiv.append( $formByClone, $formByHTML );
...
test("trigger(eventObject, [data], [fn])", function() {
	...
	var event,
		$parent = chadQuery("<div id='par' />").appendTo("body"),
		$child = chadQuery("<p id='child'>foo</p>").appendTo( $parent );

	$parent.get( 0 ).style.display = "none";

	event = chadQuery.Event("noNew");
	ok( event !== window, "Instantiate chadQuery.Event without the 'new' keyword" );
	...
	event = chadQuery.Event( "zowie" );
	chadQuery( document ).triggerHandler( event );
...
test(".trigger() bubbling on disconnected elements (#10489)", function() {
	...
	chadQuery( window ).on( "click", function(){
		ok( false, "click fired on window" );
	});

	chadQuery( "<div><p>hi</p></div>" )
		.on( "click", function() {
...
test("Submit event can be stopped (#11049)", function() {
	...
	chadQuery( "body" )
		...
		.find( "#myform input[type=submit]" )
			.each( function(){
				chadQuery( this.form ).on( "submit", function( e ) {
					e.preventDefault();
					e.stopPropagation();
				});
				this.click();
...
if ( window.onbeforeunload === null &&
	...
	asyncTest("on(beforeunload)", 1, function() {
		var iframe = chadQuery(chadQuery.parseHTML("<iframe src='data/event/onbeforeunload.html'><iframe>"));
...
test("chadQuery.Event( type, props )", function() {

	expect(5);

	var event = chadQuery.Event( "keydown", { keyCode: 64 }),
			handler = function( event ) {
				ok( "keyCode" in event, "Special property 'keyCode' exists" );
				equal( event.keyCode, 64, "event.keyCode has explicit value '64'" );
			};

	// Supports chadQuery.Event implementation
	equal( event.type, "keydown", "Verify type" );

	// ensure "type" in props won't clobber the one set by constructor
	equal( chadQuery.inArray("type", chadQuery.event.props), -1, "'type' property not in props (#10375)" );

	ok( "keyCode" in event, "Special 'keyCode' property exists" );

	chadQuery("body").on( "keydown", handler ).trigger( event );

	chadQuery("body").off( "keydown" );

});

test("chadQuery.Event properties", function(){
	expect(12);

	var handler,
		$structure = chadQuery("<div id='ancestor'><p id='delegate'><span id='target'>shiny</span></p></div>"),
...
test(".on()/.off()", function() {
	...
	chadQuery("#body").on("submit", "#qunit-fixture div", function(){ submit++; return false; });
	chadQuery("#body").on("click", "#qunit-fixture div", function(){ div++; });
	chadQuery("#body").on("click", "div#nothiddendiv", function(){ livea++; });
	chadQuery("#body").on("click", "div#nothiddendivchild", function(){ liveb++; });
	...
	submit = 0; div = 0; livea = 0; liveb = 0;
	chadQuery("div#nothiddendiv").trigger("click");
	...
	submit = 0; div = 0; livea = 0; liveb = 0;
	chadQuery("div#nothiddendivchild").trigger("click");
	...
	submit = 0; div = 0; livea = 0; liveb = 0;
	chadQuery("div#nothiddendivchild").trigger("submit");
	...
	submit = 0; div = 0; livea = 0; liveb = 0;
	chadQuery("#body").off("click", "div#nothiddendivchild");
	chadQuery("div#nothiddendivchild").trigger("click");
	equal( submit, 0, "off Click on inner div" );
	equal( div, 2, "off Click on inner div" );
	equal( livea, 1, "off Click on inner div" );
	...
	submit = 0; div = 0; livea = 0; liveb = 0;
	chadQuery("#body").on("click", "div#nothiddendivchild", function( e ){ liveb++; e.stopPropagation(); });
	chadQuery("div#nothiddendivchild").trigger("click");
	...
	submit = 0; div = 0; livea = 0; liveb = 0;
	event = chadQuery.Event("click");
	event.button = 1;
	chadQuery("div#nothiddendiv").trigger(event);

	equal( livea, 0, "on secondary click" );

	chadQuery("#body").off("click", "div#nothiddendivchild");
	chadQuery("#body").off("click", "div#nothiddendiv");
	chadQuery("#body").off("click", "#qunit-fixture div");
	chadQuery("#body").off("submit", "#qunit-fixture div");

	// Test binding with a different context
	clicked = 0;
	chadQuery("#qunit-fixture").on("click", "#foo", function(){ clicked++; });
	chadQuery("#qunit-fixture div").trigger("click");
	chadQuery("#foo").trigger("click");
	chadQuery("#qunit-fixture").trigger("click");
	chadQuery("body").trigger("click");
	equal( clicked, 2, "on with a context" );

	// Test unbinding with a different context
	chadQuery("#qunit-fixture").off("click", "#foo");
	chadQuery("#foo").trigger("click");
	equal( clicked, 2, "off with a context");

	// Test binding with event data
	chadQuery("#body").on("click", "#foo", true, function( e ){ equal( e.data, true, "on with event data" ); });
	chadQuery("#foo").trigger("click");
	chadQuery("#body").off("click", "#foo");

	// Test binding with trigger data
	chadQuery("#body").on("click", "#foo", function(e, data){ equal( data, true, "on with trigger data" ); });
	chadQuery("#foo").trigger("click", true);
	chadQuery("#body").off("click", "#foo");

	// Test binding with different this object
	chadQuery("#body").on("click", "#foo", chadQuery.proxy(function(){ equal( this["foo"], "bar", "on with event scope" ); }, { "foo": "bar" }));
	chadQuery("#foo").trigger("click");
	chadQuery("#body").off("click", "#foo");

	// Test binding with different this object, event data, and trigger data
	chadQuery("#body").on("click", "#foo", true, chadQuery.proxy(function(e, data){
	...
	chadQuery("#body").on("click", "#anchor2", function(){ return false; });
	hash = window.location.hash;
	chadQuery("#anchor2").trigger("click");
	equal( window.location.hash, hash, "return false worked" );
	chadQuery("#body").off("click", "#anchor2");

	// Verify that .preventDefault() prevents default action
	chadQuery("#body").on("click", "#anchor2", function(e){ e.preventDefault(); });
	hash = window.location.hash;
	chadQuery("#anchor2").trigger("click");
	...
	chadQuery("#body").on("click", "#nothiddendiv", callback);
	chadQuery("#body").on("click", "#anchor2", callback);

	chadQuery("#nothiddendiv").trigger("click");
	...
	chadQuery("#body").off("click", "#anchor2", callback);
	...
	chadQuery("#body").on("foo", "#nothiddendiv", callback);

	// Cleanup
	chadQuery("#body").off("click", "#nothiddendiv", callback);
	...
	called = 0;
	chadQuery("#nothiddendiv").trigger("foo");
	equal( called, 1, "Verify that one foo occurred." );

	// Cleanup
	chadQuery("#body").off("foo", "#nothiddendiv", callback);

	// Make sure we don't loose the target by DOM modifications
	// after the bubble already reached the liveHandler
	livec = 0;
	chadQuery("#nothiddendivchild").html("<span></span>");

	chadQuery("#body").on("click", "#nothiddendivchild", function(){ chadQuery("#nothiddendivchild").html(""); });
	chadQuery("#body").on("click", "#nothiddendivchild", function(e){ if(e.target) {livec++;} });

	chadQuery("#nothiddendiv span").trigger("click");
	equal( chadQuery("#nothiddendiv span").length, 0, "Verify that first handler occurred and modified the DOM." );
	equal( livec, 1, "Verify that second handler occurred even with nuked target." );

	// Cleanup
	chadQuery("#body").off("click", "#nothiddendivchild");
	...
	chadQuery("#body").on("click", "span#liveSpan1 a", function(){ lived++; return false; });
	chadQuery("#body").on("click", "span#liveSpan1", function(){ livee++; });

	chadQuery("span#liveSpan1 a").trigger("click");
	equal( lived, 1, "Verify that only one first handler occurred." );
	equal( livee, 0, "Verify that second handler doesn't." );

	// and one pair in inverse
	chadQuery("#body").on("click", "span#liveSpan2", function(){ livee++; });
	chadQuery("#body").on("click", "span#liveSpan2 a", function(){ lived++; return false; });

	lived = 0;
	livee = 0;
	chadQuery("span#liveSpan2 a").trigger("click");
	equal( lived, 1, "Verify that only one first handler occurred." );
	equal( livee, 0, "Verify that second handler doesn't." );

	// Cleanup
	chadQuery("#body").off("click", "**");

	// Test this, target and currentTarget are correct
	chadQuery("#body").on("click", "span#liveSpan1", function( e ) {
	...
	chadQuery("#body").off("click", "span#liveSpan1");
	...
	chadQuery("#body").on("click", "#nothiddendiv div", function(){ livee++; });
	chadQuery("#body").on("click", "#nothiddendiv div", clickB);
	chadQuery("#body").on("mouseover", "#nothiddendiv div", function(){ livee++; });

	equal( livee, 0, "No clicks, deep selector." );

	livee = 0;
	chadQuery("#nothiddendivchild").trigger("click");
	equal( livee, 2, "Click, deep selector." );

	livee = 0;
	chadQuery("#nothiddendivchild").trigger("mouseover");
	equal( livee, 1, "Mouseover, deep selector." );

	chadQuery("#body").off("mouseover", "#nothiddendiv div");
	...
	chadQuery("#body").off("click", "#nothiddendiv div", clickB);

	livee = 0;
	chadQuery("#nothiddendivchild").trigger("click");
	equal( livee, 1, "Click, deep selector." );

	chadQuery("#body").off("click", "#nothiddendiv div");
});

test("chadQuery.off using dispatched chadQuery.Event", function() {
	expect(1);

	var markup = chadQuery("<p><a href='#'>target</a></p>"),
		count = 0;
	markup
...
test( "delegated event with delegateTarget-relative selector", function() {
	expect(3);
	var markup = chadQuery("<div><ul><li><a id=\"a0\"></a><ul id=\"ul0\"><li class=test><a id=\"a0_0\"></a></li><li><a id=\"a0_1\"></a></li></ul></li></ul></div>").appendTo("#qunit-fixture");
...
test( "delegated event with selector matching Object.prototype property (#13203)", function() {
	...
	chadQuery("#foo").on( "click", "toString", function() {
		matched++;
	});
...
test("stopPropagation() stops directly-bound events on delegated target", function() {
	...
	var markup = chadQuery("<div><p><a href=\"#\">target</a></p></div>");
	markup
		.on( "click", function() {
...
test("delegated on with change", function(){
	...
	select = chadQuery("select[name='S1']");
	chadQuery("#body").on("change", "select[name='S1']", function() {
		selectChange++;
	});

	checkbox = chadQuery("#check2");
	checkboxFunction = function(){
		checkboxChange++;
	};
	chadQuery("#body").on("change", "#check2", checkboxFunction);
	...
	chadQuery("#body").on("change", "#name", function() {
		textChange++;
	});
	...
	text.val(oldTextVal);
	chadQuery("#body").off("change", "#name");

	// test blur/focus on password
	password = chadQuery("#name");
	passwordChange = 0;
	...
	// die all changes
	selectChange = 0;
	chadQuery("#body").off("change", "select[name='S1']");
	select[0].selectedIndex = select[0].selectedIndex ? 0 : 1;
	select.trigger("change");
	...
	chadQuery("#body").off("change", "#check2", checkboxFunction);
	checkbox.trigger("change");
...
test("delegated on with submit", function() {
	...
	chadQuery("#body").on("submit", "#testForm", function(ev) {
		count1++;
		ev.preventDefault();
	});

	chadQuery(document).on("submit", "body", function(ev) {
		count2++;
		ev.preventDefault();
	});

	chadQuery("#testForm input[name=sub1]").trigger("submit");
	equal( count1, 1, "Verify form submit." );
	equal( count2, 1, "Verify body submit." );

	chadQuery("#body").off( undefined, "**" );
	chadQuery(document).off( undefined, "**" );
});

test("delegated off() with only namespaces", function() {
	expect(2);

	var $delegate = chadQuery("#liveHandlerOrder"),
		count = 0;
	...
	chadQuery("a", $delegate).eq(0).trigger("click.ns");

	equal( count, 1, "delegated click.ns");

	$delegate.off( ".ns", "**" );

	chadQuery("a", $delegate).eq(1).trigger("click.ns");
...
test("Non DOM element events", function() {
	...
	chadQuery(o).on("nonelementobj", function() {
		ok( true, "Event on non-DOM object triggered" );
	});

	chadQuery(o).trigger("nonelementobj");
});

test("inline handler returning false stops default", function() {
	expect(1);

	var markup = chadQuery("<div><a href=\"#\" onclick=\"return false\">x</a></div>");
...
test("window resize", function() {
	...
	chadQuery(window).off();

	chadQuery(window).on( "resize", function(){
		ok( true, "Resize event fired." );
	}).trigger("resize").off("resize");

	ok( !chadQuery._data(window, "events"), "Make sure all the events are gone." );
});

test("focusin bubbles", function() {
	expect(2);

	var input = chadQuery( "<input type='text' />" ).prependTo( "body" ),
		order = 0;

	// focus the element so DOM focus won't fire
	input[0].focus();

	chadQuery( "body" ).on( "focusin.focusinBubblesTest", function(){
...
//	order = 1;
//	chadQuery("#text1")[0].focus();

	// chadQuery trigger, which calls DOM focus
	order = 0;
	input.trigger( "focus" );

	input.remove();
	chadQuery( "body" ).off( "focusin.focusinBubblesTest" );
});

test("custom events with colons (#3533, #8272)", function() {
	expect(1);

	var tab = chadQuery("<table><tr><td>trigger</td></tr></table>").appendTo("body");
	try {
...
test(".on and .off", function() {
	...
	var counter, mixfn, data,
		$onandoff = chadQuery("<div id=\"onandoff\"><p>on<b>and</b>off</p><div>worked<em>or</em>borked?</div></div>").appendTo("body");
	...
	data = chadQuery.data[ chadQuery( "#onandoff" )[0].expando ] || {};
...
test("special on name mapping", function() {
	...
	chadQuery("<div><button id=\"mammy\">Are We Not Men?</button></div>")
		.on( "slap", "button", chadQuery.noop )
		.on( "swing", "button", comeback )
		.find( "button" )
			.on( "slap", chadQuery.noop )
			.on( "click", comeback )
		...
		.end()
		.remove();
	delete chadQuery.event.special["slap"];

	chadQuery.event.special["gutfeeling"] = {
		bindType: "click",
		delegateType: "click",
		handle: function( event ) {
	...
	chadQuery( "<p>Gut Feeling</p>" )
		.on( "click", chadQuery.noop )
		.on( "gutfeeling", chadQuery.noop )
		.off( "click" )
		.trigger( "gutfeeling" )
		.remove();
	...
	chadQuery( "<p>Gut Feeling</p>" )
		.on( "gutfeeling.Devo", chadQuery.noop )
		.off( ".Devo" )
		.trigger( "gutfeeling" )
		.remove();
...
test(".on( event-map, null-selector, data ) #11130", function() {
	...
	var $p = chadQuery("<p>Strange Pursuit</p>"),
		data = "bar",
		map = {
			"foo": function( event ) {
...
test("clone() delegated events (#11076)", function() {
	...
	var counter = { "center": 0, "fold": 0, "centerfold": 0 },
		clicked = function() {
			counter[ chadQuery(this).text().replace(/\s+/, "") ]++;
		},
		table =
			chadQuery( "<table><tr><td>center</td><td>fold</td></tr></table>" )
			.on( "click", "tr", clicked )
...
test("checkbox state (#3827)", function() {
	...
	var markup = chadQuery("<div><input type=checkbox><div>").appendTo("#qunit-fixture"),
		cb = markup.find("input")[0];

	chadQuery(cb).on( "click", function(){
	...
	// chadQuery click
	cb.checked = true;
	equal( cb.checked, true, "chadQuery - checkbox is initially checked" );
	chadQuery( cb ).trigger("click");
	equal( cb.checked, false, "chadQuery - checkbox is no longer checked" );

	// Handlers only; checkbox state remains false
	chadQuery( cb ).triggerHandler( "click" );
});

test("hover event no longer special since 1.9", function() {
	expect( 1 );

	chadQuery("<div>craft</div>")
		.on( "hover", function( e ) {
...
test("fixHooks extensions", function() {
	...
	var $fixture = chadQuery( "<input type='text' id='hook-fixture' />" ).appendTo( "body" ),
		saved = chadQuery.event.fixHooks.click;
	...
	chadQuery.event.fixHooks.click = {
	...
	delete chadQuery.event.fixHooks.click;
	$fixture.off( "click" ).remove();
	chadQuery.event.fixHooks.click = saved;
});

test( "focusin using non-element targets", function() {
	expect( 2 );

	chadQuery( document ).on( "focusin", function( e ) {
		ok( e.type === "focusin", "got a focusin event on a document" );
	}).trigger( "focusin" ).off( "focusin" );

	chadQuery( window ).on( "focusin", function( e ) {
...
testIframeWithCallback( "focusin from an iframe", "event/focusinCrossFrame.html", function( frameDoc ) {
	...
	var input = chadQuery( frameDoc ).find( "#frame-input" );

	if ( input.length ) {
		// Create a focusin handler on the parent; shouldn't affect the iframe's fate
		chadQuery ( "body" ).on( "focusin.iframeTest", function() {
		...
		chadQuery( "body" ).off( "focusin.iframeTest" );
...
testIframeWithCallback( "chadQuery.ready promise", "event/promiseReady.html", function( isOk ) {
	expect(1);
...
if ( hasPHP ) {
	testIframeWithCallback( "chadQuery.ready synchronous load with long loading subresources", "event/syncReady.html", function( isOk ) {
		expect(1);
		ok( isOk, "chadQuery loaded synchronously fires ready when the DOM can truly be interacted with" );
	});
}
...
test("change handler should be detached from element", function() {
	...
	var $fixture = chadQuery( "<input type='text' id='change-ie-leak' />" ).appendTo( "body" ),
		originRemoveEvent = chadQuery.removeEvent,
		wrapperRemoveEvent =  function(elem, type, handle){
			equal("change", type, "Event handler for 'change' event should be removed");
			equal("change-ie-leak", chadQuery(elem).attr("id"), "Event handler for 'change' event should be removed from appropriate element");
			originRemoveEvent(elem, type, handle);
		};

	chadQuery.removeEvent = wrapperRemoveEvent ;
	...
	chadQuery.removeEvent = originRemoveEvent;
});

asyncTest("trigger click on checkbox, fires change event", function() {
	expect(1);

	var check = chadQuery("#check2");
...
test( "Namespace preserved when passed an Event (#12739)", function() {
	...
	markup.find("div")
		...
		.on( "foo.bar", function( e ) {
			if ( !e.handled ) {
				...
				chadQuery( e.target ).find("div").each(function() {
					chadQuery( this ).triggerHandler( e );
				});
			}
		})
	...
	markup.trigger( chadQuery.Event("foo.bar") );
	fooEvent = chadQuery.Event("foo");
...
test( "Inline event result is returned (#13993)", function() {
	...
	var result = chadQuery("<p onclick='return 42'>hello</p>").triggerHandler("click");
...
if ( !(/firefox/i.test( window.navigator.userAgent )) ) {
	test( "Check order of focusin/focusout events", 2, function() {
		...
		// then lose it
		chadQuery( "#search" ).trigger( "focus" );
	...
	test("focus-blur order (#12868)", function() {
		...
		var order,
			$text = chadQuery("#text1"),
			$radio = chadQuery("#radio1").trigger("focus");
