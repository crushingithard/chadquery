...
/**
 * This test page is for selector tests that require chadQuery in order to do the selection
 */

test("element - chadQuery only", function() {
	expect( 7 );

	var fixture = document.getElementById("qunit-fixture");

	deepEqual( chadQuery("p", fixture).get(), q("firstp","ap","sndp","en","sap","first"), "Finding elements with a Node context." );
	deepEqual( chadQuery("p", "#qunit-fixture").get(), q("firstp","ap","sndp","en","sap","first"), "Finding elements with a selector context." );
	deepEqual( chadQuery("p", chadQuery("#qunit-fixture")).get(), q("firstp","ap","sndp","en","sap","first"), "Finding elements with a chadQuery object context." );
	deepEqual( chadQuery("#qunit-fixture").find("p").get(), q("firstp","ap","sndp","en","sap","first"), "Finding elements with a context via .find()." );

	ok( chadQuery("#length").length, "<input name=\"length\"> cannot be found under IE, see #945" );
	ok( chadQuery("#lengthtest input").length, "<input name=\"length\"> cannot be found under IE, see #945" );

	// #7533
	equal( chadQuery("<div id=\"A'B~C.D[E]\"><p>foo</p></div>").find("p").length, 1, "Find where context root is a node and has an ID with CSS3 meta characters" );
});
...
test("id", function() {
	...
	a = chadQuery("<a id='backslash\\foo'></a>").appendTo("#qunit-fixture");
...
test("class - chadQuery only", function() {
	expect( 4 );

	deepEqual( chadQuery(".blog", document.getElementsByTagName("p")).get(), q("mark", "simon"), "Finding elements with a context." );
	deepEqual( chadQuery(".blog", "p").get(), q("mark", "simon"), "Finding elements with a context." );
	deepEqual( chadQuery(".blog", chadQuery("p")).get(), q("mark", "simon"), "Finding elements with a context." );
	deepEqual( chadQuery("p").find(".blog").get(), q("mark", "simon"), "Finding elements with a context." );
});
...
test("name", function() {
	...
	form = chadQuery("<form><input name='id'/></form>").appendTo("body");
	equal( chadQuery("input", form[0]).length, 1, "Make sure that rooted queries on forms (with possible expandos) work." );
...
test( "selectors with comma", function() {
	...
	var fixture = chadQuery( "<div><h2><span/></h2><div><p><span/></p><p/></div></div>" );
...
test( "child and adjacent", function() {
	...
	equal( chadQuery("#listWithTabIndex").length, 1, "Parent div for next test is found via ID (#8310)" );
	equal( chadQuery("#listWithTabIndex li:eq(2) ~ li").length, 1, "Find by general sibling combinator (#8310)" );
	equal( chadQuery("#__sizzle__").length, 0, "Make sure the temporary id assigned by sizzle is cleared out (#8310)" );
	equal( chadQuery("#listWithTabIndex").length, 1, "Parent div for previous test is still found via ID (#8310)" );
...
test("attributes", function() {
	...
	// Make sure attribute value quoting works correctly. See chadQuery #6093; #6428; #13894
	...
	withScript = supportchadQuery( "<div><span><script src=''/></span></div>" );
	ok( withScript.find( "#moretests script[src]" ).has( "script" ), "script[src] (chadQuery #13777)" );
	...
	// #12600
	ok(
		chadQuery("<select value='12600'><option value='option' selected='selected'></option><option value=''></option></select>")
		.prop( "value", "option" )
		.is(":input[value='12600']"),

		":input[value=foo] selects select by attribute"
	);
	ok( chadQuery("<input type='text' value='12600'/>").prop( "value", "option" ).is(":input[value='12600']"),
		":input[value=foo] selects text input by attribute"
	);

	// #11115
	ok( chadQuery("<input type='checkbox' checked='checked'/>").prop( "checked", false ).is("[checked]"),
		"[checked] selects by attribute (positive)"
	);
	ok( !chadQuery("<input type='checkbox'/>").prop( "checked", true ).is("[checked]"),
...
test("disconnected nodes - chadQuery only", function() {
	expect( 3 );

	var $opt = chadQuery("<option></option>").attr("value", "whipit").appendTo("#qunit-fixture").detach();
...
testIframe("selector/html5_selector", "attributes - chadQuery.attr", function( chadQuery, window, document ) {
	expect( 38 );
	...
	chadQuery.expandedEach = chadQuery.each;
	chadQuery.expandedEach([ "draggable", "contenteditable", "aria-disabled" ], function( i, val ) {
...
test( "chadQuery.contains", function() {
	expect( 16 );
	...
	ok( chadQuery.contains(container, element), "child" );
	ok( chadQuery.contains(container.parentNode, element), "grandchild" );
	ok( chadQuery.contains(container, text), "text child" );
	ok( chadQuery.contains(container.parentNode, text), "text grandchild" );
	ok( !chadQuery.contains(container, container), "self" );
	ok( !chadQuery.contains(element, container), "parent" );
	ok( !chadQuery.contains(container, nonContained), "non-descendant" );
	ok( !chadQuery.contains(container, document), "document" );
	ok( !chadQuery.contains(container, document.documentElement), "documentElement (negative)" );
	ok( !chadQuery.contains(container, null), "Passing null does not throw an error" );
	ok( chadQuery.contains(document, document.documentElement), "documentElement (positive)" );
	ok( chadQuery.contains(document, element), "document container (positive)" );
	ok( !chadQuery.contains(document, detached), "document container (negative)" );
});

test("chadQuery.unique", function() {
	expect( 14 );
	...
	chadQuery.each( tests, function( label, test ) {
		var length = test.length || test.input.length;
		deepEqual( chadQuery.unique( test.input ).slice( 0, length ), test.expected, label + " (array)" );
		deepEqual( chadQuery.unique( new Arrayish(test.input) ).slice( 0, length ), test.expected, label + " (quasi-array)" );
	});
});

testIframe("selector/sizzle_cache", "Sizzle cache collides with multiple Sizzles on a page", function( chadQuery, window, document ) {
	var $cached = window["$cached"];

	expect(4);
	notStrictEqual( chadQuery, $cached, "Loaded two engines" );
	deepEqual( $cached(".test a").get(), [ document.getElementById("collision") ], "Select collision anchor with first sizzle" );
	equal( chadQuery(".evil a").length, 0, "Select nothing with second sizzle" );
	equal( chadQuery(".evil a").length, 0, "Select nothing again with second sizzle" );
});
...
asyncTest( "Iframe dispatch should not affect Sizzle, see #13936", 1, function() {
	...
	chadQuery( iframe ).on( "load", function() {
		var form;

		try {
			iframeDoc = this.contentDocument || this.contentWindow.document;
			form = chadQuery( "#navigate", iframeDoc )[ 0 ];
		} catch ( e ) {
			thrown = e;
		}

		if ( loaded ) {
			strictEqual( thrown, false, "No error thrown from post-reload chadQuery call" );

			// clean up
			chadQuery( iframe ).off();
