...
chadQuery.each( tests, function( strFlags, resultString ) {

		var objectFlags = {};

		chadQuery.each( strFlags.split( " " ), function() {
			if ( this.length ) {
				objectFlags[ this ] = true;
			}
		});

		chadQuery.each( filters, function( filterLabel ) {
			...
			chadQuery.each({
				...
				test( "chadQuery.Callbacks( " + showFlags( flags ) + " ) - " + filterLabel, function() {
					...
					output = "X";
					cblist = chadQuery.Callbacks( flags );
...
test( "chadQuery.Callbacks( options ) - options are copied", function() {
	...
	var options = {
		...
		cb = chadQuery.Callbacks( options ),
		count = 0,
		fn = function() {
...
test( "chadQuery.Callbacks.fireWith - arguments are copied", function() {

	expect( 1 );

	var cb = chadQuery.Callbacks("memory"),
		args = ["hello"];
...
test( "chadQuery.Callbacks.remove - should remove all instances", function() {

	expect( 1 );

	var cb = chadQuery.Callbacks();
...
test( "chadQuery.Callbacks.has", function() {
...
test( "chadQuery.Callbacks() - adding a string doesn't cause a stack overflow", function() {

	expect( 1 );

	chadQuery.Callbacks().add( "hello world" );
