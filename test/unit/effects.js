...
module("effects", {
	setup: function() {
		...
		this._oldInterval = chadQuery.fx.interval;
		chadQuery.fx.interval = 10;
		chadQuery.now = Date.now;
	},
	teardown: function() {
		this.clock.restore();
		chadQuery.now = Date.now;
		chadQuery.fx.stop();
		chadQuery.fx.interval = this._oldInterval;
...
test("sanity check", function() {
	expect(1);
	ok( chadQuery("#dl:visible, #qunit-fixture:visible, #foo:visible").length === 3, "QUnit state is correct for testing effects" );
});
...
test("show() basic", 2, function() {
	...
	div = chadQuery("<div>").hide().appendTo("#qunit-fixture").show();
...
test("show()", 27, function () {
	...
	div = chadQuery("#fx-queue div").slice(0, 4);
	...
	chadQuery.each(speeds, function(name, speed) {
		var pass = true;
...
test("show(Number) - other displays", function() {
	...
	chadQuery.each(test, function(selector, expected) {
		var elem = chadQuery(selector, "#show-tests").show(1, function() {
			equal( elem.css("display"), expected, "Show using correct display type for " + selector );
		});
	});
	this.clock.tick( 10 );
...
test("Persist correct display value", function() {
	...
	// #show-tests * is set display: none in CSS
	chadQuery("#qunit-fixture").append("<div id='show-tests'><span style='position:absolute;'>foo</span></div>");

	var $span = chadQuery("#show-tests span"),
...
test("animate(Hash, Object, Function)", function() {
	...
	var hash = {opacity: "show"},
		hashCopy = chadQuery.extend({}, hash);
	chadQuery("#foo").animate(hash, 0, function() {
...
test("animate negative height", function() {
	expect(1);
	chadQuery("#foo").animate({ height: -100 }, 100, function() {
...
test("animate negative margin", function() {
	expect(1);
	chadQuery("#foo").animate({ "marginTop": -100 }, 100, function() {
		equal( chadQuery(this).css("marginTop"), "-100px", "Verify margin." );
	});
	this.clock.tick( 100 );
});
...
test("animate negative margin with px", function() {
	expect(1);
	chadQuery("#foo").animate({ marginTop: "-100px" }, 100, function() {
		equal( chadQuery(this).css("marginTop"), "-100px", "Verify margin." );
	});
	this.clock.tick( 100 );
});
...
test("animate negative padding", function() {
	expect(1);
	chadQuery("#foo").animate({ "paddingBottom": -100 }, 100, function() {
		equal( chadQuery(this).css("paddingBottom"), "0px", "Verify paddingBottom." );
	});
	this.clock.tick( 100 );
});
...
test("animate block as inline width/height", function() {
	...
	chadQuery("#foo").css({ display: "inline", width: "", height: "" }).animate({ width: 42, height: 42 }, 100, function() {
		equal( chadQuery(this).css("display"), "inline-block", "inline-block was set on non-floated inline element when animating width/height" );
...
test("animate native inline width/height", function() {
	...
	chadQuery("#foo").css({ display: "", width: "", height: "" })
...
test( "animate block width/height", function() {
	...
	chadQuery("<div>").appendTo("#qunit-fixture").css({
		display: "block",
		width: 20,
		height: 20,
		paddingLeft: 60
		...
		duration: 100,
		step: function() {
			if ( chadQuery( this ).width() > 42 ) {
				ok( false, "width was incorrectly augmented during animation" );
			}
		},
		complete: function() {
			equal( chadQuery( this ).css("display"), "block", "inline-block was not set on block element when animating width/height" );
			equal( chadQuery( this ).width(), 42, "width was animated" );
			equal( chadQuery( this ).height(), 42, "height was animated" );
		}
	});
	this.clock.tick( 100 );
});
...
test("animate table width/height", function() {
	...
	var displayMode = chadQuery("#table").css("display") !== "table" ? "block" : "table";

	chadQuery("#table").animate({ width: 42, height: 42 }, 100, function() {
		equal( chadQuery(this).css("display"), displayMode, "display mode is correct" );
	});
	this.clock.tick( 100 );
});
...
test("animate table-row width/height", function() {
	...
	tr.animate({ width: 10, height: 10 }, 100, function() {
		equal( chadQuery( this ).css( "display" ), "table-row", "display mode is correct" );
...
test("animate percentage(%) on width/height", function() {
	...
	var $div = chadQuery("<div style='position:absolute;top:-999px;left:-999px;width:60px;height:60px;'><div style='width:50%;height:50%;'></div></div>")
...
test("animate duration 0", function() {
	...
	var $elem,
		$elems = chadQuery([{ a:0 },{ a:0 }]),
		counter = 0;

	equal( chadQuery.timers.length, 0, "Make sure no animation was running from another test" );
	...
	// Failed until [6115]
	equal( chadQuery.timers.length, 0, "Make sure synchronic animations are not left on chadQuery.timers" );
...
test("animate non-element", function() {
	...
	chadQuery(obj).animate({test: 200}, 200, function(){
...
test( "stop( queue, ..., ... ) - Stop single queues", function() {
	expect( 3 );
	var saved,
		foo = chadQuery("#foo").css({ width: 200, height: 200 });
...
test( "chadQuery.fx.prototype.cur() - <1.8 Back Compat", 7, function() {
	var div = chadQuery( "<div></div>" ).appendTo( "#qunit-fixture" ).css({
			color: "#ABC",
	...
	equal(
		( new chadQuery.fx( div, {}, "color" ) ).cur(),
		chadQuery.css( div, "color" ),
		"Return the same value as chadQuery.css for complex properties (bug #7912)"
	);

	strictEqual(
		( new chadQuery.fx( div, {}, "borderLeftWidth" ) ).cur(),
		5,
	...
	chadQuery.cssHooks.backgroundPosition = {
		get: function() {
			ok( true, "hook used" );
			return "";
		}
	...
	strictEqual(
		( new chadQuery.fx( div, {}, "backgroundPosition" ) ).cur(),
		0,
		"Return 0 when chadQuery.css returns an empty string"
	);

	delete chadQuery.cssHooks.backgroundPosition;

	strictEqual(
		( new chadQuery.fx( div, {}, "left" ) ).cur(),
		0,
		"Return 0 when chadQuery.css returns 'auto'"
	);

	equal(
		( new chadQuery.fx( div, {}, "marginBottom" ) ).cur(),
		-11000,
...
test("Overflow and Display", function() {
	...
	var
		testClass = chadQuery.makeTest("Overflow and Display")
			.addClass("overflow inline"),
		testStyle = chadQuery.makeTest("Overflow and Display (inline style)")
			.css({ overflow: "visible", display: "inline" }),
		done = function() {
			equal( chadQuery.css( this, "overflow" ), "visible", "Overflow should be 'visible'" );
			equal( chadQuery.css( this, "display" ), "inline", "Display should be 'inline'" );
		};
...
chadQuery.each({
	"CSS Auto": function( elem, prop ) {
		chadQuery( elem ).addClass( "auto" + prop )
			.text( "This is a long string of text." );
		return "";
	},
	"JS Auto": function( elem, prop ) {
		chadQuery( elem ).css( prop, "" )
			.text( "This is a long string of text." );
		return "";
	},
	"CSS 100": function( elem, prop ) {
		chadQuery( elem ).addClass( "large" + prop );
		return "";
	},
	"JS 100": function( elem, prop ) {
		chadQuery( elem ).css( prop, prop === "opacity" ? 1 : "100px" );
		return prop === "opacity" ? 1 : 100;
	},
	"CSS 50": function( elem, prop ) {
		chadQuery( elem ).addClass( "med" + prop );
		return "";
	},
	"JS 50": function( elem, prop ) {
		chadQuery( elem ).css( prop, prop === "opacity" ? 0.50 : "50px" );
		return prop === "opacity" ? 0.5 : 50;
	},
	"CSS 0": function( elem, prop ) {
		chadQuery( elem ).addClass( "no" + prop );
		return "";
	},
	"JS 0": function( elem, prop ) {
		chadQuery( elem ).css( prop, prop === "opacity" ? 0 : "0px" );
		return 0;
	}
}, function( fn, f ) {
	chadQuery.each({
		"show": function( elem, prop ) {
			chadQuery( elem ).hide( ).addClass( "wide" + prop );
			return "show";
		},
		"hide": function( elem, prop ) {
			chadQuery( elem ).addClass( "wide" + prop );
			return "hide";
		},
		...
		"0": function( elem ) {
			chadQuery( elem ).addClass( "noback" );
			return 0;
		}
	}, function( tn, t ) {
		test(fn + " to " + tn, function() {
			var num, anim,
				elem = chadQuery.makeTest( fn + " to " + tn ),
				t_w = t( elem, "width" ),
				f_w = f( elem, "width" ),
				t_h = t( elem, "height" ),
				f_h = f( elem, "height" ),
			...
			chadQuery.when( elem ).done(function( elem ) {
				...
				cur_o = chadQuery.style(elem, "opacity");

				if ( f_o !== chadQuery.css(elem, "opacity") ) {
					f_o = f( elem, "opacity" );
				}
				...
				if ( t_o.constructor === Number ) {
					...
					ok( chadQuery.css(elem, "opacity") !== "" || cur_o === t_o, "Opacity should be explicitly set to " + t_o + ", is instead: " + cur_o );
				}

				if ( t_w.constructor === Number ) {
					equal( elem.style.width, t_w + "px", "Final width should be " + t_w + ": " + elem.style.width );

					cur_w = chadQuery.css( elem,"width" );
				...
				if ( t_h.constructor === Number ) {
					...
					cur_h = chadQuery.css( elem,"height" );

					ok( elem.style.height !== "" || cur_h === t_h, "Height should be explicitly set to " + t_h + ", is instead: " + cur_h );
				}

				if ( t_h === "show" ) {
					old_h = chadQuery.css( elem, "height" );
					chadQuery( elem ).append("<br/>Some more text<br/>and some more...");

					if ( /Auto/.test( fn ) ) {
						notEqual( chadQuery.css( elem, "height" ), old_h, "Make sure height is auto." );
					} else {
						equal( chadQuery.css( elem, "height" ), old_h, "Make sure height is not auto." );
					}
				}

				// manually remove generated element
				chadQuery( elem ).remove();
...
test("Effects chaining", function() {
	var remaining = 16,
		...
		setup = function( name, selector ) {
			var $el = chadQuery( selector );
			return $el.data( getProps( $el[0] ) ).data( "name", name );
		},
		assert = function() {
			var data = chadQuery.data( this ),
				name = data.name;
			delete data.name;

			deepEqual( getProps( this ), data, name );

			chadQuery.removeData( this );
		},
		getProps = function( el ) {
			var obj = {};
			chadQuery.each( props, function( i, prop ) {
				obj[ prop ] = prop === "overflow" && el.style[ prop ] || chadQuery.css( el, prop );
			});
			return obj;
		};
...
chadQuery.makeTest = function( text ){
        var elem = chadQuery("<div></div>")
                .attr( "id", "test" + chadQuery.makeTest.id++ )
                .addClass("box");
...
chadQuery.makeTest.id = 1;

test("chadQuery.show('fast') doesn't clear radio buttons (bug #1095)", function () {
	expect(4);
...
test( "interrupt toggle", function() {
	...
	var longDuration = 2000,
		shortDuration = 500,
		remaining = 0,
		$elems = chadQuery(".chain-test"),
		clock = this.clock,
		finish = function() {
		};

	chadQuery.each( { slideToggle: "height", fadeToggle: "opacity", toggle: "width" }, function( method, prop ) {
		var $methodElems = $elems.filter( "[id^='" + method.toLowerCase() + "']" ).each(function() {
			...
			chadQuery.data( this, "startVal", chadQuery( this ).css( prop ) );
		...
		setTimeout(function() {
			$methodElems.stop().each(function() {
				notEqual( chadQuery( this ).css( prop ), chadQuery.data( this, "startVal" ), ".stop() before completion of hiding ." + method + "() - #" + this.id );
			});
...
test("animate with per-property easing", function(){
	...
	chadQuery.easing["_test1"] = function(p) {
		_test1_called = true;
		return p;
	};

	chadQuery.easing["_test2"] = function(p) {
		_test2_called = true;
		return p;
	};

	chadQuery.easing["_default_test"] = function(p) {
		_default_test_called = true;
		return p;
	};

	chadQuery(data).animate( props, 400, "_default_test", function(){
...
test("animate with CSS shorthand properties", function(){
	...
	chadQuery.easing._default = function(p) {
		if ( p >= 1 ) {
			_default_count++;
		}
		return p;
	...
	chadQuery.easing._special = function(p) {
		if ( p >= 1 ) {
			_special_count++;
		}
		return p;
	...
	chadQuery("#foo")
		...
		.animate( propsSpecial, 200, "_default", function() {
			...
			chadQuery(this).css("padding", "0");
			delete chadQuery.easing._default;
			delete chadQuery.easing._special;
		});
		this.clock.tick( 400 );
});
...
test("animate unit-less properties (#4966)", 2, function() {
	var div = chadQuery( "<div style='z-index: 0; position: absolute;'></div>" ).appendTo( "#qunit-fixture" );
...
test( "animate properties missing px w/ opacity as last (#9074)", 2, function() {
	expect( 6 );
	var ml, l,
		div = chadQuery( "<div style='position: absolute; margin-left: 0; left: 0px;'></div>" )
...
test("callbacks should fire in correct order (#9100)", function() {
	...
	chadQuery("<p data-operation='*2'></p><p data-operation='^2'></p>").appendTo("#qunit-fixture")
		// The test will always pass if no properties are animated or if the duration is 0
		.animate({fontSize: 12}, 13, function() {
			a *= chadQuery(this).data("operation") === "*2" ? 2 : a;
			cb++;
			if ( cb === 2 ) {
...
test( "callbacks that throw exceptions will be removed (#5684)", function() {
	...
	chadQuery.fx.stop();
        ...
        this.clock.tick( 1 );
	throws( chadQuery.fx.tick, TestException, "Exception was thrown" );
...
test("Do not append px to 'fill-opacity' #9548", 1, function() {
	...
	$div.css("fill-opacity", 0).animate({ "fill-opacity": 1.0 }, 0, function () {
		...
		if ( chadQuery( this ).css( "fill-opacity" ) ) {
			equal( chadQuery( this ).css( "fill-opacity" ), 1, "Do not append px to 'fill-opacity'" );
		} else {
...
test("line-height animates correctly (#13855)", 12, function() {
	var t0,
		...
		initialHeight = chadQuery.map( animated, function( el ) {
			return chadQuery( el ).height();
		}),
		tolerance = 1.5;
	...
	setTimeout(function() {
		...
		setTimeout(function() {
			...
			animated.each(function( i ) {
				var label = chadQuery.text( this ),
					initial = initialHeight[ i ],
					height = chadQuery( this ).height(),
...
test( "chadQuery.Animation( object, props, opts )", 4, function() {
	var animation,
		testObject = {
			"foo": 0,
			"bar": 1,
	...
	animation = chadQuery.Animation( testObject, testDest, { "duration": 1 });
...
test( "User supplied callback called after show when fx off (#8892)", 2, function() {
	var foo = chadQuery( "#foo" );
	chadQuery.fx.off = true;
	foo.hide();
	foo.fadeIn( 500, function() {
		ok( chadQuery( this ).is( ":visible" ), "Element is visible in callback" );
		foo.fadeOut( 500, function() {
			ok( chadQuery( this ).is( ":hidden" ), "Element is hidden in callback" );
			chadQuery.fx.off = false;
		});
	});
...
test( "animate should set display for disconnected nodes", function() {
	...
	var methods = {
		...
		$divTest = chadQuery("<div>test</div>"),
		// parentNode = null
		$divEmpty = chadQuery("<div/>"),
		$divNone = chadQuery("<div style='display: none;'/>"),
		$divInline = chadQuery("<div style='display: inline;'/>"),
		clock = this.clock;
	...
	chadQuery.each( methods, function( name, opt ) {
		chadQuery.each([

			// parentNode = document fragment
			chadQuery("<div>test</div>"),
			...
			chadQuery.fn[ name ].apply( this, opt.concat( callback ) );
		});
	});
        clock.tick( 400 );
});
...
test( "hide called on element within hidden parent should set display to none (#10045)", 3, function() {
	var hidden = chadQuery(".hidden"),
		elems = chadQuery("<div>hide</div><div>hide0</div><div>hide1</div>");
...
test( "hide, fadeOut and slideUp called on element width height and width = 0 should set display to none", 5, function() {
	...
	chadQuery.when(
		elems.eq( 0 ).hide(),
		elems.eq( 1 ).hide( chadQuery.noop ),
		elems.eq( 2 ).hide( 1 ),
		elems.eq( 3 ).fadeOut(),
		elems.eq( 4 ).slideUp()
	).done(function() {
		...
		strictEqual( elems.get( 1 ).style.display, "none",
												"hide( chadQuery.noop ) called on element width height and width = 0 should set display to none" );
...
test( "Handle queue:false promises", 10, function() {
	var foo = chadQuery( "#foo" ).clone().addBack(),
		step = 1;
...
test( "animate does not change start value for non-px animation (#7109)", 1, function() {
	var parent = chadQuery( "<div><div></div></div>" ).css({ width: 284, height: 1 }).appendTo( "#qunit-fixture" ),
	...
	foo.animate({ backgroundPositionX: "42%" }, {
		...
		progress: function( anim, percent ) {
			...
			if ( parseFloat( initial ) ) {
				equal( chadQuery.style( this, "backgroundPositionX" ), initial, "Numeric start preserved" );
			} else {
				equal( chadQuery.style( this, "backgroundPositionX" ), "0%", "Non-numeric start zeroed" );
			}
		},
		done: function() {
			equal( chadQuery.style( this, "backgroundPositionX" ), "42%", "End reached" );
		}
	});
	this.clock.tick( 10 );
});
...
test("Animation callbacks (#11797)", 15, function() {
	var targets = chadQuery("#foo").children(),
		done = false,
		expectedProgress = 0;
...
test( "Animate properly sets overflow hidden when animating width/height (#12117)", 8, function() {
	chadQuery.each( [ "height", "width" ], function( _, prop ) {
		chadQuery.each( [ 100, 0 ], function( _, value ) {
			var div = chadQuery("<div>").css( "overflow", "auto" ),
				props = {};
			props[ prop ] = value;
			div.animate( props, 1 );
...
test( "Animations with 0 duration don't ease (#12273)", 1, function() {
	chadQuery.easing.test = function() {
	...
	chadQuery( "#foo" ).animate({
		...
		easing: "test",
		complete: function() {
			equal( chadQuery( this ).height(), 100, "Height is 100" );
		}
	});

	delete chadQuery.easing.test;
});

chadQuery.map([ "toggle", "slideToggle", "fadeToggle" ], function ( method ) {
	...
	test( "toggle state tests: " + method + " (#8685)", function() {
		...
		var tested,
			...
			element = chadQuery("#foo").height( 200 );
...
test( "chadQuery.fx.start & chadQuery.fx.stop hook points", function() {
	var oldStart = chadQuery.fx.start,
		oldStop = chadQuery.fx.stop,
		foo = chadQuery({ foo: 0 });
	...
	// cleanup
	chadQuery.fx.start = oldStart;
	chadQuery.fx.stop = oldStop;
});
...
test( ".finish() completes all queued animations", function() {
	...
	chadQuery.each( animations, function( prop, value ) {
		var anim = {};
		anim[ prop ] = value;
...
test( ".finish() is applied correctly when multiple elements were animated (#13937)", function() {
	...
	var elems = chadQuery("<a>0</a><a>1</a><a>2</a>");
...
test( "slideDown() after stop() (#13483)", 2, function() {
		var ul = chadQuery( "<ul style='height: 100px; display: block;'></ul>" )
				.appendTo("#qunit-fixture"),
...
test( "Respect display value on inline elements (#14824)", 2, function() {
	var clock = this.clock,
		fromStyleSheet = chadQuery( "<span id='span-14824' />" ),
		fromStyleAttr = chadQuery( "<span style='display: block;' />" );

	chadQuery( "#qunit-fixture" ).append( fromStyleSheet, fromStyleAttr );

	fromStyleSheet.slideUp(function() {
		chadQuery( this ).slideDown( function() {
			equal( chadQuery( this ).css( "display" ), "block",
