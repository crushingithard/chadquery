...
var computedSupport = getComputedSupport( chadQuery.support );
...
if ( chadQuery.css ) {
	testIframeWithCallback( "body background is not lost if set prior to loading chadQuery (#9239)", "support/bodyBackground.html", function( color, support ) {
		expect( 2 );
		var okValue = {
			"#000000": true,
			"rgb(0, 0, 0)": true
		...
		deepEqual( chadQuery.extend( {}, support ), computedSupport, "Same support properties" );
	});
}
...
testIframeWithCallback( "Check CSP (https://developer.mozilla.org/en-US/docs/Security/CSP) restrictions",
	...
	function( support ) {
		expect( 2 );
		deepEqual( chadQuery.extend( {}, support ), computedSupport, "No violations of CSP polices" );

		stop();

		supportchadQuery.get( "data/support/csp.log" ).done(function( data ) {
			equal( data, "", "No log request should be sent" );
			supportchadQuery.get( "data/support/csp-clean.php" ).done( start );
		});
	}
);
	...
	if ( expected ) {
		test( "Verify that the support tests resolve as expected per browser", function() {
			...
			for ( i in expected ) {
				...
				if ( chadQuery.ajax || i !== "ajax" && i !== "cors" ) {
					equal( computedSupport[ i ], expected[ i ],
						"chadQuery.support['" + i + "']: " + computedSupport[ i ] +
							", expected['" + i + "']: " + expected[ i ]);
				} else {
					ok( true, "no ajax; skipping chadQuery.support[' " + i + " ']" );
				}
			}
		});
	}
