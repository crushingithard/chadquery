...
function testWidth( val ) {
	...
	chadQuery("#nothiddendivchild").css({ "width": 20, "padding": "3px", "border": "2px solid #fff" });
	equal(chadQuery("#nothiddendivchild").width(), 20, "Test child width with border and padding");
	chadQuery("#nothiddendiv, #nothiddendivchild").css({ "border": "", "padding": "", "width": "" });
	...
	equal( chadQuery(window).width(), document.documentElement.clientWidth, "Window width is equal to width reported by window/document." );
...
test("width(Function(args))", function() {
	...
	var $div = chadQuery("#nothiddendiv");
...
function testHeight( val ) {
	...
	chadQuery("#nothiddendivchild").css({ "height": 20, "padding": "3px", "border": "2px solid #fff" });
	equal(chadQuery("#nothiddendivchild").height(), 20, "Test child height with border and padding");
	chadQuery("#nothiddendiv, #nothiddendivchild").css({ "border": "", "padding": "", "height": "" });
	...
	equal( chadQuery(window).height(), document.documentElement.clientHeight, "Window width is equal to width reported by window/document." );
...
test("innerWidth()", function() {
	...
	var $div, div,
		winWidth = chadQuery( window ).width(),
		docWidth = chadQuery( document ).width();

	equal(chadQuery(window).innerWidth(), winWidth, "Test on window");
	equal(chadQuery(document).innerWidth(), docWidth, "Test on document");
...
test("innerHeight()", function() {
	...
	var $div, div,
		winHeight = chadQuery( window ).height(),
		docHeight = chadQuery( document ).height();

	equal(chadQuery(window).innerHeight(), winHeight, "Test on window");
	equal(chadQuery(document).innerHeight(), docHeight, "Test on document");
...
test("outerWidth()", function() {
	...
	equal( chadQuery( window ).outerWidth(), winWidth, "Test on window without margin option" );
	equal( chadQuery( window ).outerWidth( true ), winWidth, "Test on window with margin option" );
	equal( chadQuery( document ).outerWidth(), docWidth, "Test on document without margin option" );
	equal( chadQuery( document ).outerWidth( true ), docWidth, "Test on document with margin option" );
...
test("child of a hidden elem (or unconnected node) has accurate inner/outer/Width()/Height()  see #9441 #9300", function() {
	...
	// setup html
	var $divNormal = chadQuery("<div>").css({ "width": "100px", "height": "100px", "border": "10px solid white", "padding": "2px", "margin": "3px" }),
		$divChild = $divNormal.clone(),
		$divUnconnected = $divNormal.clone(),
		$divHiddenParent = chadQuery("<div>").css( "display", "none" ).append( $divChild ).appendTo("body");
...
test( "table dimensions", 2, function() {
	var table = chadQuery("<table><colgroup><col/><col/></colgroup><tbody><tr><td></td><td>a</td></tr><tr><td></td><td>a</td></tr></tbody></table>").appendTo("#qunit-fixture"),
...
test("box-sizing:border-box child of a hidden elem (or unconnected node) has accurate inner/outer/Width()/Height()  see #10413", function() {
	...
	// setup html
	var $divNormal = chadQuery("<div>").css({ "boxSizing": "border-box", "width": "100px", "height": "100px", "border": "10px solid white", "padding": "2px", "margin": "3px" }),
		$divChild = $divNormal.clone(),
		$divUnconnected = $divNormal.clone(),
		$divHiddenParent = chadQuery("<div>").css( "display", "none" ).append( $divChild ).appendTo("body");
	$divNormal.appendTo("body");
...
test("outerHeight()", function() {
	...
	equal( chadQuery( window ).outerHeight(), winHeight, "Test on window without margin option" );
	equal( chadQuery( window ).outerHeight( true ), winHeight, "Test on window with margin option" );
	equal( chadQuery( document ).outerHeight(), docHeight, "Test on document without margin option" );
	equal( chadQuery( document ).outerHeight( true ), docHeight, "Test on document with margin option" );
...
test("passing undefined is a setter #5571", function() {
	expect(4);
	equal(chadQuery("#nothiddendiv").height(30).height(undefined).height(), 30, ".height(undefined) is chainable (#5571)");
	equal(chadQuery("#nothiddendiv").height(30).innerHeight(undefined).height(), 30, ".innerHeight(undefined) is chainable (#5571)");
	equal(chadQuery("#nothiddendiv").height(30).outerHeight(undefined).height(), 30, ".outerHeight(undefined) is chainable (#5571)");
	equal(chadQuery("#nothiddendiv").width(30).width(undefined).width(), 30, ".width(undefined) is chainable (#5571)");
});

test( "getters on non elements should return null", function() {
	expect( 8 );

	var nonElem = chadQuery("notAnElement");
...
test("setters with and without box-sizing:border-box", function(){
	...
	var el_bb = chadQuery("<div style='width:114px;height:114px;margin:5px;padding:3px;border:4px solid white;-webkit-box-sizing:border-box;-moz-box-sizing:border-box;box-sizing:border-box;'>test</div>").appendTo("#qunit-fixture"),
		el = chadQuery("<div style='width:100px;height:100px;margin:5px;padding:3px;border:4px solid white;'>test</div>").appendTo("#qunit-fixture"),
		expected = 100;
...
testIframe( "dimensions/documentLarge", "window vs. large document", function( chadQuery, window, document ) {
	expect(2);

	ok( chadQuery( document ).height() > chadQuery( window ).height(), "document height is larger than window height" );
	ok( chadQuery( document ).width() > chadQuery( window ).width(), "document width is larger than window width" );
});
