...
chadQuery.each( [ "", " - new operator" ], function( _, withNew ) {

	function createDeferred( fn ) {
		return withNew ? new chadQuery.Deferred( fn ) : chadQuery.Deferred( fn );
	}

	test( "chadQuery.Deferred" + withNew, function() {
		...
		createDeferred(function( defer ) {
			...
			chadQuery.each( promise, function( key ) {
				if ( !chadQuery.isFunction( promise[ key ] ) ) {
					ok( false, key + " is a function (" + chadQuery.type( promise[ key ] ) + ")" );
				}
		...
		chadQuery.expandedEach( "resolve reject".split(" "), function( _, change ) {
			createDeferred(function( defer ) {
...
test( "chadQuery.Deferred - chainability", function() {

	var defer = chadQuery.Deferred();

	expect( 10 );

	chadQuery.expandedEach = chadQuery.each;
	chadQuery.expandedEach( "resolve reject notify resolveWith rejectWith notifyWith done fail progress always".split(" "), function( _, method ) {
		var object = {
			m: defer[ method ]
		};
...
test( "chadQuery.Deferred.then - filtering (done)", function() {
	...
	chadQuery.Deferred().reject().then(function() {
		ok( false, "then should not be called on reject" );
	});

	chadQuery.Deferred().resolve().then( chadQuery.noop ).done(function( value ) {
		strictEqual( value, undefined, "then done callback can return undefined/null" );
	});
});

test( "chadQuery.Deferred.then - filtering (fail)", function() {
	...
	chadQuery.Deferred().resolve().then( null, function() {
		ok( false, "then should not be called on resolve" );
	});

	chadQuery.Deferred().reject().then( null, chadQuery.noop ).fail(function( value ) {
		strictEqual( value, undefined, "then fail callback can return undefined/null" );
	});
});

test( "chadQuery.Deferred.then - filtering (progress)", function() {
...
test( "chadQuery.Deferred.then - deferred (done)", function() {
...
test( "chadQuery.Deferred.then - deferred (fail)", function() {
...
test( "chadQuery.Deferred.then - deferred (progress)", function() {
...
test( "chadQuery.Deferred.then - context", function() {
	...
	chadQuery.Deferred().resolveWith( context, [ 2 ] ).then(function( value ) {
		return value * 3;
	}).done(function( value ) {
	...
	chadQuery.Deferred().resolve().then(function() {
		return chadQuery.Deferred().resolveWith(context);
	}).done(function() {
...
test( "chadQuery.when", function() {
	...
	chadQuery.each({
		...
		ok(
			chadQuery.isFunction(
				chadQuery.when( value ).done(function( resolveValue ) {
	...
	ok(
		chadQuery.isFunction(
			chadQuery.when().done(function( resolveValue ) {
	...
	chadQuery.when( chadQuery.Deferred().resolveWith( context ) ).done(function() {
		strictEqual( this, context, "when( promise ) propagates context" );
	});

	chadQuery.each([ 1, 2, 3 ], function( k, i ) {

		chadQuery.when( cache || chadQuery.Deferred(function() {
				this.resolve( i );
			})
		).done(function( value ) {
...
test( "chadQuery.when - joined", function() {

	expect( 119 );

	var deferreds = {
			value: 1,
			success: chadQuery.Deferred().resolve( 1 ),
			error: chadQuery.Deferred().reject( 0 ),
			futureSuccess: chadQuery.Deferred().notify( true ),
			futureError: chadQuery.Deferred().notify( true ),
			notify: chadQuery.Deferred().notify( true )
		},
		willSucceed = {
			value: true,
			success: true,
	...
	chadQuery.each( deferreds, function( id1, defer1 ) {
		chadQuery.each( deferreds, function( id2, defer2 ) {
			var shouldResolve = willSucceed[ id1 ] && willSucceed[ id2 ],
				...
				code = id1 + "/" + id2,
				context1 = defer1 && chadQuery.isFunction( defer1.promise ) ? defer1.promise() : undefined,
				context2 = defer2 && chadQuery.isFunction( defer2.promise ) ? defer2.promise() : undefined;

			chadQuery.when( defer1, defer2 ).done(function( a, b ) {
				if ( shouldResolve ) {
