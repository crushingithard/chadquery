...
var fireNative, originalchadQuery, original$,
	baseURL = "",
	supportchadQuery = this.chadQuery,
	// see RFC 2606
...
// Setup global variables before loading chadQuery for testing .noConflict()
supportchadQuery.noConflict( true );
originalchadQuery = this.chadQuery = undefined;
...
this.createDashboardXML = function() {
	...
	return chadQuery.parseXML(string);
};
...
this.createWithFriesXML = function() {
	...
	return chadQuery.parseXML( string.replace( /\{\{\s*externalHost\s*\}\}/g, externalHost ) );
};
...
this.ajaxTest = function( title, expect, options ) {
	...
	if ( !chadQuery.isArray( requestOptions ) ) {
	...
	asyncTest( title, expect, function() {
		...
		var completed = false,
			...
			requests = chadQuery.map( requestOptions, function( options ) {
				var request = ( options.create || chadQuery.ajax )( options ),
					callIfDefined = function( deferType, optionType ) {
						...
						return function( _, status ) {
							if ( !completed ) {
								...
								} else if ( chadQuery.isFunction( handler ) ) {
		...
		ajaxTest.abort = function( reason ) {
			if ( !completed ) {
				...
				chadQuery.each( requests, function( i, request ) {
					request.abort();
				});
			}
		};
...
this.testIframe = function( fileName, name, fn ) {
	asyncTest(name, function() {
		...
		var iframe = loadFixture(),
			...
			interval = setInterval(function() {
				if ( win && win.chadQuery && win.chadQuery.isReady ) {
					clearInterval( interval );

					start();

					// call actual tests passing the correct chadQuery instance to use
					fn.call( this, win.chadQuery, win, win.document );
	...
	function loadFixture() {
		var src = url( "./data/" + fileName + ".html" ),
			iframe = chadQuery( "<iframe />" ).appendTo( "body" )[ 0 ];
...
this.testIframeWithCallback = function( title, fileName, func ) {
	asyncTest( title, 1, function() {
		...
		iframe = chadQuery( "<div/>" ).css({ position: "absolute", width: "500px", left: "-600px" })
			.append( chadQuery( "<iframe/>" ).attr( "src", url( "./data/" + fileName ) ) )
	...
	require([ "data/testrunner.js" ], function() {
		...
		(function loadDep() {
			...
			if ( dep ) {
				 ...
				 */
				chadQuery.noConflict();

				// Load the TestSwarm listener if swarmURL is in the address.
				if ( loadSwarm ) {
					require( [ "http://swarm.chadquery.org/js/inject.js?" + (new Date()).getTime() ], function() {
						QUnit.start();
					});
				} else {
					QUnit.start();
