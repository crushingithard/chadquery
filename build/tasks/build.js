/**
 * Special concat/build task to handle various chadQuery build requirements
...
module.exports = function( grunt ) {
	...
	function convert( name, path, contents ) {
		...
		// Sizzle treatment
			...
			// Ignore chadQuery's exports (the only necessary one)
			if ( name !== "chadquery" ) {
				contents = contents
		...
		if ( (amdName = grunt.option( "amd" )) != null && /^exports\/amd$/.test( name ) ) {
			if (amdName) {
				grunt.log.writeln( "Naming chadQuery with AMD name: " + amdName );
			} else {
				grunt.log.writeln( "AMD name now anonymous" );
			}
			// Remove the comma for anonymous defines
			contents = contents
				.replace( /(\s*)"chadquery"(\,\s*)/, amdName ? "$1\"" + amdName + "\"$2" : "" );
	...
	function() {
		...
		if ( !optIn ) {
			...
			config.rawText.chadquery = "define([" + (included.length ? included.join(",") : "") + "]);";
		}
