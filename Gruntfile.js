module.exports = function( grunt ) {
	...
	grunt.initConfig({
		...
		compare_size: {
			files: [ "dist/chadquery.js", "dist/chadquery.min.js" ],
			options: {
				compress: {
		...
		bowercopy: {
			...
			src: {
				files: {
					...
					"src/sizzle/test/chadquery.js": "sizzle/test/chadquery.js"
				}
			},
			tests: {
				options: {
		...
		uglify: {
			all: {
				files: {
					"dist/chadquery.min.js": [ "dist/chadquery.js" ]
				},
				options: {
					preserveComments: false,
					sourceMap: "dist/chadquery.min.map",
					sourceMappingURL: "chadquery.min.map",
					report: "min",
					beautify: {
						ascii_only: true
					},
					banner: "/*! chadQuery v<%= pkg.version %> | " +
						"(c) 2005, <%= grunt.template.today('yyyy') %> chadQuery Foundation, Inc. | " +
						"chadquery.org/license */",
					compress: {
						hoist_funs: false,
						loops: false,
						unused: false
	...
	// Integrate chadQuery specific tasks
