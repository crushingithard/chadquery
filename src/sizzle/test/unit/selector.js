...
// #### NOTE: ####
// chadQuery should not be used in this module
// except for DOM manipulation
// If chadQuery is mandatory for the selection, move the test to chadquery/test/unit/selector.js
// Use t() or Sizzle()
// ###############

/*
	======== QUnit Reference ========
	http://docs.chadquery.com/QUnit
...
test("element", function() {
	...
	deepEqual( Sizzle( "em + :not(:has(*)):not(:empty), foo", siblingTest ), [ siblingNext ],
		"Non-qSA path correctly sets detached context for sibling selectors (chadQuery #14351)" );
	...
	html = chadQuery( html ).appendTo( document.body );
...
test("XML Document Selectors", function() {
	...
	xml = chadQuery.parseXML("<?xml version='1.0' encoding='UTF-8'?><root><elem id='1'/></root>");
	equal( Sizzle( "elem:not(:has(*))", xml ).length, 1,
		"Non-qSA path correctly handles numeric ids (chadQuery #14142)" );
});
...
test("broken", function() {
	...
	attrbad = chadQuery("<input type='hidden' value='2' name='foo.baz' id='attrbad1'/><input type='hidden' value='2' name='foo[baz]' id='attrbad2'/>").appendTo("#qunit-fixture");
...
test("id", function() {
	...
	fiddle = chadQuery("<div id='fiddle\\Foo'><span id='fiddleSpan'></span></div>").appendTo("#qunit-fixture");
...
test("class", function() {
	...
	div = chadQuery("<div><svg width='200' height='250' version='1.1' xmlns='http://www.w3.org/2000/svg'><rect x='10' y='10' width='30' height='30' class='foo'></rect></svg></div>")[0];
...
test("attributes", function() {
	...
	t( "script[src] (chadQuery #13777)", "#moretests script[src]", ["script-src"] );
...
test("pseudo - (first|last|only)-(child|of-type)", function() {
	...
	var secondChildren = chadQuery("p:nth-child(2)").before("<div></div>");
...
test("pseudo - misc", function() {
	...
	chadQuery.each( [ "button", "submit", "reset" ], function( i, type ) {
		var els = chadQuery(
...
test("pseudo - position", function() {
	...
	// chadQuery #12526
	var context = chadQuery("#qunit-fixture").append("<div id='chadquery12526'></div>")[0];
	deepEqual( Sizzle( ":last", context ), q("chadquery12526"), "Post-manipulation positional" );
});

test("pseudo - form", function() {
	expect( 10 );

	var extraTexts = chadQuery("<input id=\"impliedText\"/><input id=\"capitalText\" type=\"TEXT\">").appendTo("#form");
...
test("pseudo - :lang", function() {
	...
	// XML
	foo = chadQuery( "response", xml )[0];
	anchor = chadQuery( "#seite1", xml )[0];
...
test("caching", function() {
	...
	t( "Deep ancestry caching in post-positional element matcher (chadQuery #14657)",
		"#qunit-fixture a:lt(3):parent",
		[ "simon1", "google", "groups" ] );
});

asyncTest( "Iframe dispatch should not affect Sizzle, see chadQuery #13936", 1, function() {
	var loaded = false,
		thrown = false,
		iframe = document.getElementById("iframe"),
		iframeDoc = iframe.contentDocument || iframe.contentWindow.document;
...
test("select() with pre-compiled function", function() {
	...
	chadQuery.each([ "#qunit-fixture #first", "ol#listWithTabIndex > li[tabindex]", "#liveSpan1" ],
	function( i, selector ) {
