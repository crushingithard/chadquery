...
function testAttr( doc ) {
	...
	if ( doc ) {
		...
		el = chadQuery( "<input type='checkbox' checked='checked' />" )[0];
	}
...
test("Sizzle.attr (XML)", function() {
	testAttr( chadQuery.parseXML("<root/>") );
});
...
if ( chadQuery("<svg xmlns='http://www.w3.org/2000/svg' version='1.1' height='1' width='1'><g/></svg>")[0].firstChild ) {
	test("Sizzle.contains in SVG (chadQuery #10832)", function() {
		expect( 4 );
...
testIframeWithCallback( "Sizzle.uniqueSort works cross-window (chadQuery #14381)", "mixed_sort.html", deepEqual );
